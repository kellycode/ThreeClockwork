...
angular.module("clockworkApp.clockworkObjectStore", []).factory("objectStore", [
    "objectUtils",
    function (objectUtils) {
        return {
            objectSelect: function (event, threeScene) {
                ...
                mouse.x =
                    ((event.clientX - threeScene.renderer.domElement.offsetLeft) /
                        threeScene.renderer.domElement.width) *
                        2 -
                    1;
                mouse.y =
                    -(
                        (event.clientY - threeScene.renderer.domElement.offsetTop) /
                        threeScene.renderer.domElement.height
                    ) *
                        2 +
                    1;
            ...
            _loadSimpleSkybox: function (template) {
                ...
                var materialArray = [];
                for (var i = 0; i < 6; i++) var loader = new THREE.TextureLoader();
                materialArray.push(
            ...
            _loadThreeType: function (mesh) {
                switch (mesh.geometry.type) {
                    ...
                    case "PlaneBufferGeometry":
                        console.log(
                            "THREE.PlaneBufferGeometry is not used at this time as it doesn't support textures"
                        );
                        break;
                    default:
                ...
                for (var i = 0; i < sceneData.length; i++) {
                    ...
                    switch (sceneData[i].type) {
                        ...
                        case "Collada":
                            ...
                            break;
                        // The following were removed a while back when this
                        // stopped wanting to be less of an editor and more of a game

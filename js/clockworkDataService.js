"use strict";
angular.module("clockworkApp.clockworkDataService", []).factory("dataService", [
    "$http",
    "templater",
    function ($http, templater) {
        return {
            loadSceneJSON: function (sceneUrl, loadFromFile) {
                // check if we have localStorage json first
                let localStoreData = localStorage.getItem("save_scene_x1");
                // if no localStoareg json, get the default data from a file
                if (localStoreData === null) {
                    ...
                    return Promise.resolve().then(function () {
                        return JSON.parse(localStoreData);
                    });
                }
            },
